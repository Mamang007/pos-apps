...
export function PODetail({ po, onBack, onStatusUpdate }: PODetailProps) {
  ...
  const handleUpdateStatus = async (newStatus: POStatus) => {
    if (newStatus === "RECEIVED") {
      if (
        !confirm(
          "Are you sure you want to mark this PO as RECEIVED? This will increase product stocks.",
        )
      ) {
        return;
      }
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="rounded-xl border border-border bg-card p-6 shadow-sm">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 pb-6 border-b border-border">
              ...
              <span
                className={`inline-flex items-center rounded-full px-3 py-1 text-sm font-semibold border ${getStatusColor(po.status as POStatus)}`}
              >
                {po.status}
              </span>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 mb-8">
              ...
              <div className="sm:text-right">
                ...
                <p className="font-medium text-foreground mb-3">
                  {format(new Date(po.orderDate), "dd MMMM yyyy HH:mm")}
                </p>
        ...
        <div className="space-y-6">
          <div className="rounded-xl border border-border bg-card p-6 shadow-sm">
            ...
            <div className="space-y-3">
              ...
              {po.status === "RECEIVED" && (
                <div className="p-4 rounded-lg bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800 text-sm flex items-start gap-3">
                  <CheckCircle2 className="h-5 w-5 mt-0.5 shrink-0" />
                  <p>
                    This order has been received and stock levels have been
                    updated. No further changes can be made.
                  </p>
                </div>
              )}

              {po.status === "CANCELLED" && (
                <div className="p-4 rounded-lg bg-rose-50 dark:bg-rose-900/20 border border-rose-100 dark:border-rose-800 text-sm flex items-start gap-3">
